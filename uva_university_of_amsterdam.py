...
log_file_path = os.path.join(
    ...
    "temp",
    "uva_university_of_amsterdam.log",
)
log.basicConfig(
    level=log.INFO,
...
def compose_and_send_email(recipient_email, recipient_name, positions_list, utils_path):
    ...
    email_text = compose_email(
        recipient_name, "UvA University of Amsterdam", emails_text, utils_path
    )
    # send email with the generated text
    send_email(
        recipient_email,
        "PhD Positions from UvA University of Amsterdam",
        email_text,
        "html",
    )
...
if __name__ == "__main__":
    ...
    log.info("Getting UvA University of Amsterdam positions info.")
    positions_db = PositionsDatabase(
        host=db_host, user=db_user, password=db_password, db=db_name, port=db_port
    )
    positions_cnx = positions_db.connect_to_database()
    positions = positions_db.get_positions("uva_nl")
    positions_cnx.close()
    ...
    for customer in customers:
        ...
        for position in positions:
            ...
            if search_result:
                id = f"{customer.email}_{'uva_nl'}_{position.id}"
                if not tracking_emails_db.check_if_id_exist(
                    ...
                    related_positions.append(
                        [
                            ...
                            TrackingEmails(id, "uva_nl", customer.email, position.id),
                        ]
                    )
