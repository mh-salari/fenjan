...
log_file_path = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "temp", "university_of_tampere.log"
)
log.basicConfig(
    level=log.INFO,
...
class PositionsDatabase:
    ...
    def get_positions(self, table_name):
        ...
        positions = []
        for (id, title, text, url) in cursor:
            positions.append(Position(id, title, text, url))
...
def compose_and_send_email(recipient_email, recipient_name, positions, utils_path):
    """
    ...
    positions_text = []
    for title, url, matched_keywords, _ in positions:
    ...
    email_text = compose_email(
        recipient_name, "University of Tampere", positions_text, utils_path
    )
    # send email with the generated text
    send_email(
        recipient_email, "PhD Positions from University of Tampere", email_text, "html"
    )
...
if __name__ == "__main__":
    ...
    log.info("Getting University of Tampere positions info.")
    positions_db = PositionsDatabase(
        host=db_host, user=db_user, password=db_password, db=db_name, port=db_port
    )
    positions_cnx = positions_db.connect_to_database()
    positions = positions_db.get_positions("tuni_fi")
    ...
    for customer in customers:
        ...
        for position in positions:
            search_result = search_for_keywords(
                position.text, keywords=customer_keywords
            )
            if search_result and "Doctoral Researcher" in position.title:
                id = f"{customer.email}_{'tuni_fi'}_{position.id}"
                if not tracking_emails_db.check_if_id_exist(
                    ...
                    related_positions.append(
                        [
                            ...
                            position.url,
                            ", ".join(search_result),
                            TrackingEmails(id, "tuni_fi", customer.email, position.id),
                        ]
                    )
        if related_positions:
            ...
            for _, _, _, email_data in related_positions:
